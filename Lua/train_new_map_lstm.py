...
def create_model(keep_prob=0.6):
    ...
    input_map  =  BatchNormalization(batch_input_shape=(batch_size, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS))
    branch_map = Conv2D(24, kernel_size=(5, 5), strides=(2, 2), activation='relu')(input_map)
    ...
    # CNN for frame
    #input_Frame  = Input(shape=(batch_size, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS))
    input_frame  =  BatchNormalization(batch_input_shape=(batch_size, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS))
    branch_frame = TimeDistributed(Conv2D(24, kernel_size=(5, 5), strides=(2, 2), activation='relu'))(input_frame)
...
def load_training_data(track):
    ...
    z_train = z_train[0:len(z_train)-len(z_train)%batch_size]
    X_val = X_val[0:len(X_val)-len(X_val)%batch_size-100]
    y_val = y_val[0:len(y_val)-len(y_val)%batch_size-100]
    z_val = z_val[0:len(z_val)-len(z_val)%batch_size-100]
...
if __name__ == '__main__':
    ...
    X_train = X_train.reshape(2114, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS)
    y_train = y_train.reshape(2114, 5)
    z_train = z_train.reshape(2114, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS)
    X_val = X_val.reshape(218, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS)
    y_val = y_val.reshape(218, 5)
    z_val = z_val.reshape(218, 5, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS)
    ...
    model.fit([X_train, z_train], y_train, batch_size=batch_size, epochs=epochs,
              shuffle=False, validation_data=([X_val, z_val], y_val), callbacks=[checkpointer, earlystopping])
