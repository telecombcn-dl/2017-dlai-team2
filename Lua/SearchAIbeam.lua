...
local UIDIR = util.getTMPDir() .. '\\' .. util.generateUUID();
os.execute('mkdir ' .. UIDIR)

-- The save state will be temporarily stored in this file when performing a search.
local STATE_FILE = UIDIR .. '\\root.state'
...
for k = 1, BEAM_WIDTH, 1 do
  ...
  local BEAM_STATE_FILE = UIDIR .. '\\' .. k .. '.state'
  local TEMP_FILE = UIDIR .. '\\' .. k .. 'temp.state'
  local before_state = UIDIR .. '\\' .. k .. 'before.state'
