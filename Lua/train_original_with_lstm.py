...
def create_model(keep_prob=0.6, batch_size = 50):
    model = Sequential()
...
def load_training_data(track):
    ...
    #X_train = X_train[0:len(X_train)-int(((len(X_train)/timestep)%batch_size)*timestep)]
    #y_train = y_train[0:len(y_train)-int(((len(y_train)/timestep)%batch_size)*timestep)]
...
if __name__ == '__main__':
    ...
    X_train = X_train.reshape(10150, 1, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS)
    y_train = y_train.reshape(10150, 1)
    X_val = X_val.reshape(1150, 1, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS)
    y_val = y_val.reshape(1150, 1)
