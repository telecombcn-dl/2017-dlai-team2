...
from train_consecutive import create_model, is_valid_track_code, INPUT_WIDTH, INPUT_HEIGHT, INPUT_CHANNELS
        ...
        for line in self.rfile:
            ...
            if message.startswith("PREDICTFROMCLIPBOARD"):
                ...
                if im != None:
                    ...
                    CURR_IM = prepare_image(im)
                    if FIRST == True:
                        PREV_IM = CURR_IM
                        FIRST = False
                    prediction = model.predict([CURR_IM, PREV_IM], batch_size=1)[0]
                    PREV_IM = CURR_IM
